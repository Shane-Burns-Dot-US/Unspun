...
nodes, edges = [], []
for f in sorted(glob.glob(os.path.join(CAT, '0[1-6]-*.yaml'))):
    data = yaml.safe_load(open(f))
    cmap = scan_context(f)
    for name, meta in data.items():
        if not isinstance(meta, dict): continue
        nodes.append({'id': name, 'kind': 'entity', 'context': cmap.get(name),
...
kg = {'meta': {'generated_from': 'docs/data-model/catalog/*.yaml + authored logic layer',
...
lines = ['# Entities by context (generated)',
         '',
